...
public class ColorBlindFilter : MonoBehaviour
{
    ...
    void LoadSettingsFromJson()
    {
        ...
        {
            // Parsez le JSON pour récupérer la valeur de colorBlindIndex
            GameSettings gameSettings = JsonUtility.FromJson<GameSettings>(json);

            // Assurez-vous que la valeur de colorBlindIndex est valide
            if (gameSettings.colorBlindIndex >= 0 && gameSettings.colorBlindIndex < (int)ColorBlindMode.Achromatomaly + 1)
            {
                // Appliquer la valeur de colorBlindIndex récupérée depuis le fichier JSON
                mode = (ColorBlindMode)gameSettings.colorBlindIndex;
            }
        }
    }
    ...
    {
        // Mettez à jour le mode de daltonisme avec la valeur actuelle du dropdown
        mode = (ColorBlindMode)FindObjectOfType<SettingsMenu>().colorBlindDropdown.value;
    }
}
